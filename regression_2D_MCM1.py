...
print (os.getcwd())
if(hpc==False):
    path1="D:\\Mega\\phd\\classification_datasets\\label_partition\\label_noise\\MCM3"
else:
...
#    i=100
    ...
    samples = 1000
    data = datasets.make_regression(n_samples=samples,n_features=2, n_informative=2, n_targets=3, bias=1, effective_rank=None,
                                    tail_strength=0.5, noise=4, shuffle=True, coef=False, random_state=dataset_name)    
...
#    plt.scatter(X, Y, marker='o')
    ...
    Y, me, std_dev=standardize(Y)
#    plt.scatter(X[:, 0], X[:, 1], marker='o', c=Y)
    #%%
    num_batches=5
    kf=KFold(n_splits=num_batches, random_state=1, shuffle=True)
    j=0
...
#    gamma1 = [0] #hyperparameter3 (kernel parameter for non-linear classification or regression)

    epsilon1 = [1e-04,1e-03,1e-02,1e-01,1,10] #hyperparameter4 ( It specifies the epsilon-tube within which 
...
#    tol1 = [1e-04] #tolerance to cut off SGD
    ...
    update_type ='adam'
    update_type_idx = 6
    reg_type1 = {0:'l1', 1:'l2', 2:'en', 4:'ISTA'} #{0:'l1', 1:'l2', 2:'en', 4:il2, 5:'ISTA'}#ISTA: iterative soft thresholding (proximal gradient)
    reg_type = 'l1'
    reg_type_idx = 0
                ...
                for C4 in Cd:
                    for gamma in gamma1:
                        for epsilon in epsilon1:
                            t0=time()
                            mcm = MCM(C1 = C1, C2 = C2, C3 = C3, C4 = C4, problem_type = problem_type, kernel_type = kernel_type, gamma = gamma, 
                                      epsilon = epsilon, feature_ratio = feature_ratio, sample_ratio = sample_ratio, feature_sel = feature_sel, 
                                      n_ensembles = n_ensembles, batch_sz = batch_sz, iterMax1 = iterMax1, eta = eta, tol = tol, update_type = update_type, 
                            ...
                            train_acc=mcm.accuracy_regressor(yTrain,train_pred)
                            val_acc=mcm.accuracy_regressor(yVal,val_pred)
...
#                                    W1=np.append(W1,W.flatten())
                            ...
                            result=np.append(result,np.array([[train_acc, val_acc, time_elapsed, non_zero_weights, C1, C2, C3, C4, problem_type_idx, kernel_type_idx, gamma,
                                                               ...
                                                               PV_scheme_idx, n_components, do_pca_in_selection_idx]]),axis=0)
    ...
    result_pd.to_csv(path1+"/results/Train_results_%d_%s"%(dataset_name,typeAlgo)+"_%s.csv"%(dataset_type))
    max_acc=np.min(result[:,1])
    max_acc_idx=result[:,1]==max_acc
    min_sv=np.min(result[max_acc_idx,3])
    min_sv_idx=result[:,3]==min_sv
    ...
    train_acc=mcm.accuracy_regressor(yTrain,train_pred)
    test_acc=mcm.accuracy_regressor(yTest,test_pred)
#    train_f1= f1_score(yTrain, train_pred, average='weighted') 
#    test_f1 =f1_score(yTest, test_pred, average='weighted') 
    ...
    result1=np.append(result1,np.array([[train_acc, test_acc, time_elapsed, non_zero_weights, C1, C2, C3, C4, problem_type_idx, kernel_type_idx, gamma,
                                         ...
                                         n_components, do_pca_in_selection_idx]]),axis=0)
    ...
    result_pd1.to_csv(path1+"/results/Test_results_%d_%s"%(dataset_name,typeAlgo)+"_%s.csv"%(dataset_type))
    #%% plotting the graph
#    temp=xx.ravel().shape
#    xTest1=np.c_[xx.ravel(), yy.ravel(),np.ones(temp)]
#    Z=mcm.predict_classifier(xTest1,W_all,feature_indices)
...
#    plt.xticks(())
#    plt.yticks(())
#    fig_title='%d_%s_%s'%(dataset_name,dataset_type,typeAlgo)
